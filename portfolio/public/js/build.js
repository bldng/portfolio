...
module.exports = function(message, sender, type) {
  var formatted, i, item, j, len, len1, messageString, messageToDisplay, tag;
  if (type === 'linklist') {
...
(function (global){
var about_me, calcAge, contact, content, feedback, projects, say;
...
about_me = [
  {
    ...
    answer: '<ul><li><del>LinkedIn</del></li><li><del>Facebook</del></li><li><del>twitter</del></li><li><a href="https://github.com/bldng" target="_blank">Github</a></li><li><a href="https://github.com/bldng" target="_blank">codepen</a></li></ul>',
    tags: ['socialmedia']
  }, {
    ...
    answer: 'Via Email: <a href="mailto:hello@gerhardbliedung.com">hello@gerhardbliedung.com</a>' + contact,
    tags: ['contact']
  }, {
    answer: '<div class="longtext"><p><strong>Languages</strong></p><p>Human – Human</p><ul><li>German ( mother Tongue )</li><li>English ( fluent in speech and writing )</li></ul><p>Human – Machine</p><ul><li>Javascript ( Node.js, jQuery, three.js, Angular, babel )</li><li>browserify / webpack</li><li>build tools</li><li>templating</li><li>css preprocessors</li><li>Java, C++ ( Processing, Arduino)</li></ul><p><strong>Skills</strong></p><p>Software</p><ul><li>Adobe CC Suite ( Ps, Ai, In, Ae )</li><li>Sketch</li><li>Git</li><li>Cinema 4d &amp; blender</li></ul><p>Additional Skills</p><ul><li>Digital and analog illustrations</li><li>Physical Prototyping</li><li>Robotics ( Dynamixel / Arduino )</li><li>Calligraphy</li></ul></div>',
    tags: ['skills']
  }, {
    answer: 'Zurich University of the Arts,<br> graduated in 2014',
    tags: ['university']
  }, {
    answer: '<b>Bachelor of Arts in Product and Industrial Design with a Specialization in Interaction Design</b><br>Zurich University of the Arts<br><small>',
    tags: ['degree']
  }
];

module.exports = function(message) {
  var api_reply, cleanedMessage, url;
  cleanedMessage = message.replace('#', '');
  url = '/api?body=' + cleanedMessage + '&state=' + context;
  return api_reply = $.getJSON(url, function(data) {
    var allProjects, category, chanceToApologize, filteredInfo, filteredProjects, i, item, j, len, len1, random;
    if (data.intent === 'projects') {
      ...
      if (category === 'all') {
        ...
        return $(".conversation").append(say(filteredProjects, 'bot--delay', 'linklist'));
      } else if (category === 'random') {
        random = Math.floor(Math.random() * projects.length + 0);
        filteredProjects.push({
          title: projects[random].title,
          tag: projects[random].tags[0]
        });
        ...
        filteredProjects = projects.filter(function(item) {
          if (item.tags.indexOf(category) > -1) {
            return filteredProjects.push(item.title);
          }
        });
        ...
        if (filteredProjects.length > 0) {
          ...
          return $(".conversation").append(say('Sorry, there was no match for ' + category, 'bot'));
        }
      }
    } else if (data.intent === 'about_me') {
      filteredInfo = [];
      filteredInfo = about_me.filter(function(item) {
        if (item.tags.indexOf(data.category.toLowerCase()) > -1) {
          return item.answer;
        }
      });
      if (filteredInfo.length < 1) {
        return $(".conversation").append(say("Sorry, I don't know. If you want to ask him, just type &laquo; contact &raquo; ", 'bot'));
      } else {
        $(".conversation").append(say(data.reply, 'bot'));
        return $(".conversation").append(say(filteredInfo, 'bot--delay', 'numerical'));
      }
      ...
      if (global.context.length === 0) {
        global.context = 'sailor';
      }
      return $(".conversation").append(say(data.reply, 'bot--delay'));
    } else if (data.intent === 'yes_sorry') {
      global.context = '';
      $('.main').removeClass('howdoyulikethat');
      $('.rickroll').remove();
      return $(".conversation").append(say(data.reply, 'bot'));
      ...
      $(".conversation").append(say(data.reply, 'bot'));
      chanceToApologize = function() {
        global.context = '';
        return console.log('too late');
      };
      return setTimeout(chanceToApologize, 25000);
    } else if (data.intent === 'no_send') {
      global.context = '';
      return $('.send--textarea').focus();
    } else if (data.intent === 'yes_feedback') {
      global.context = '';
      return $(".conversation").append("<div class='bot feedbackform'>Would you mind sending a short Feedback? This would be awesome!</div><div class='bot numerical feedbackform'>" + feedback + "</div>");
    } else if (data.intent === 'no_feedback') {
      global.context = '';
      return $(".conversation").append(say('No problem! Maybe some other time.', 'bot'));
      ...
      $(".conversation").append(say(data.reply, 'bot'));
      return $(".conversation").append(say(allProjects, 'bot--delay', 'all-projects'));
    } else if (data.intent === 'wolfram') {
      $(".conversation").append(say('Wolfram alpha says:', 'bot'));
      return $(".conversation").append(say(data.reply, 'bot--delay', 'wolfram'));
    } else {
      return $(".conversation").append(say(data.reply, 'bot'));
    }
  });
...
},{"../../../public/js/content.json":10,"./generateMessage":1}],3:[function(require,module,exports){
(function (global){
var askAfter, content, get, greet, isMobile, itemslide, page, textFit;
...
page('/', function(ctx) {
  var fitHeader, hash, props, tellError;
  ctx.handled = true;
  hash = ctx.hash.toString();
  props = content.projects.items.filter(function(item) {
    return item.title === hash;
  });
  if (props.length > 0) {
    $(document).scrollTop(0);
    $('.content').html(templates['project']({
      'props': props[0]
    }));
    ...
    return setTimeout(fitHeader, 100);
  } else {
    ...
    if (hash.length > 0) {
      tellError = function() {
        return $(".conversation").append("<div class='bot'>Well, this is awkward. I can't find the page you wanted :(</div>");
      };
      return setTimeout(tellError, 4000);
    }
  }
});
...
$("#talk").on('submit', function(event) {
  var askFeedback, clientMessage;
  ...
  if ($('.conversation').children().length === askAfter) {
    if (global.context.length === 0 && $('#contactForm').length === 0) {
      ...
      askFeedback = function() {
        return $(".conversation").append('<div class="bot--delay">Could you do me a small favor?</div>');
      };
      return setTimeout(askFeedback, 1500);
    }
  }
});

$('.conversation').on('submit', '#contactForm', function(event) {
  var mail, messageToSend, phone;
  event.preventDefault();
  ...
  if (messageToSend.replace(/\s/g, "") !== "") {
    if (mail.test(messageToSend) || phone.test(messageToSend) || $('input[name="submitValue"]').val() === 'true') {
      ...
      return $(".conversation--input").focus();
    }
  } else {
    return $(".send--textarea").val('');
  }
});

$('.conversation').on('submit', '#feedbackForm', function(event) {
  var messageToSend;
  event.preventDefault();
  ...
  if (messageToSend.replace(/\s/g, "") !== "") {
    ...
    return $(".conversation").append('<div class="bot">I will forward it immediately! Thank you :)</div>');
  } else {
    return $(".feedback--textarea").val('');
  }
});
...
$('.conversation').on('click', '.all-projects--close', function() {
  $(this).remove();
  $('.all-projects h3, .all-projects h4').remove();
  return $('.all-projects').removeClass('all-projects').addClass('linklist');
});
...
$("video").bind("play", function() {
  var vid;
  return vid = setInterval(function() {
    if (!$("video").get(0).paused) {
      return $('.main.hidden').addClass('video');
    } else {
...
},{"../../public/js/content.json":10,"./lib/getMessage":2,"./vendor/itemslide.min.js":4,"./vendor/textFit.min.js":5,"jquery":6,"page":7}],4:[function(require,module,exports){
/*
     _ _      _       _
 ___| (_) ___| | __  (_)___
/ __| | |/ __| |/ /  | / __|
